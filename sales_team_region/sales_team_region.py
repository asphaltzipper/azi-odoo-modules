...
class res_partner(models.Model):
    ...
    @api.model
    def _lookup_section(self, state_id=False, customer=False,
                        country_id=False):
        if (state_id or country_id) and customer:
            ...
            if region_id:
                section = self.env['crm.case.section'].search(
                    [('region_id', '=', region_id)])
                if section:
                    return section
        #return 0
    ...
    def lookup_section(self, state_id=False, customer=False, country_id=False):
        return self._lookup_section(state_id, customer,
                                    country_id).id or self._default_section()
    ...
    def _require_section(self):
        ...
        for record in self:
                ...
                #return False
            if (self.env['res.users'].has_group('base.group_multi_salesteams')
                    and record.customer and not record.section_id.id):
    ...
    def _st_search(self):
        ...
        added due to Error triggered during 8.0 database update 20150320:
            Error: 'boolean' object has no attribute '_fnct_search'" while
            parsing azi-odoo-modules/sales_team_region/sales_team_view.xml:31
        """
        return [('id', 'in', [])]
...
#    section_id = fields.Many2one('crm.case.section', 'Sales Team', default=lambda self: self.with_context(self._context)._default_section(self.state_id, self.customer))
    ...
    def onchange_state(self, state_id=False, customer=False):
        ...
        if (self.env['res.users'].has_group('base.group_multi_salesteams') and
                res):
            res['value']['section_id'] = self._lookup_section(state_id,
                                                              customer)
    ...
    def onchange_country(self, country_id=False, customer=False,
        ...
        if (self.env['res.users'].has_group('base.group_multi_salesteams') and
                not state_trigger):
            return {'value': {'section_id': self._lookup_section(
...
class sales_team_region_country_group_rel(models.Model):
    ...
    _sql_constraints = [
        ('rel_country_group_uniq', 'unique(country_id)',
...
class sales_team_region(models.Model):
    ...
    @api.multi
    def onchange_region(self, states=False, countries=False,
        ...
        for state_id in states[0][2]:
            for state in self.env['res.country.state'].browse(state_id):
                ...
                for country_group in self.env['res.country.group'].search(
                        [('country_ids', 'in', state.country_id.id)]):
                    country_group_ids.add(country_group.id)
